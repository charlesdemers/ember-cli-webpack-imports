...
module.exports = class WebpackDependencyPlugin extends Plugin {
  ...
  build() {
    const outputFile = path.join(this.outputPath, `-${this.options.outputName}-bundle.js`);
  ...
  _writeEntryFile() {
    ...
    const libs = this.options.expose.map(lib => `'${lib}': require('${lib}')`);
    const contents = `module.exports = { ${libs.join(', ')} };`;
  ...
  _writeShims() {
    ...
    const defines = this.options.expose.map((lib) => `
